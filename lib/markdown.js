...
function textOf(content, withReasoning = false) {
    const parts = [];
...
export function summarizeStats(events, maxResultChars = 2000) {
    let turns = 0;
    let steps = 0;
    let toolCalls = 0;
    let toolErrors = 0;
 ...
 */
export function renderSessionMarkdown(events, meta, maxResultChars = 2000, includeInjected = false) {
    ...
    if (stats.model !== undefined)
        out.push(`- **模型**：\`${stats.model}\``);
    out.push(`- **统计**：${stats.turns} 个 turn · ${stats.steps} 个 step · ${stats.toolCalls} 次工具调用` +
    ...
    for (const event of events) {
        switch (event.type) {
            ...
            case 'user/message': {
                ...
                if (!includeInjected && kind !== 'user' && kind !== 'goal')
                    break;
    ...
    if (inTurn)
        out.push('> 会话在 turn 进行中导出（进程退出或中断）。', '');
    return out.join('\n');
}
